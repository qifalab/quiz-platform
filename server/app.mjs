...
export function createApp({
  ...
  function addBank(name, questions) {
    ...
    transact(() => {
      run('INSERT INTO banks VALUES(?,?,?)', bankId, name, Date.now());
  ...
  if (!one('SELECT id FROM banks LIMIT 1'))
    addBank(
      '入门示例题库',
      [
        {
  ...
  app.use('/api', (req, res, next) => {
    ...
    if (!['GET', 'HEAD'].includes(req.method) && req.headers.origin !== origin)
      return res.status(403).json({ error: '请求来源不正确，请刷新页面重试' });
    next();
  });
  ...
  const admin = (req, res, next) => {
    if (req.user.admin_until < Date.now())
      return res.status(401).json({ error: '请先登录题库管理' });
    next();
  };
  app.post(
    '/api/admin/login',
    ...
    (req, res) => {
      ...
      if (
        !timingSafeEqual(
          ...
          Buffer.from(hash(adminPassword)),
        )
      )
  ...
  app.get('/api/state', (req, res) => {
    const banks = all(
      `SELECT b.*, COUNT(q.id) total FROM banks b LEFT JOIN questions q ON q.bank_id=b.id GROUP BY b.id ORDER BY b.created DESC`,
    ).map((b) => ({
      ...b,
      types: all(
        'SELECT DISTINCT type FROM questions WHERE bank_id=?',
        b.id,
      ).map((r) => r.type),
      categories: all(
        'SELECT DISTINCT category FROM questions WHERE bank_id=?',
        b.id,
      ).map((r) => r.category),
      ...one(
        'SELECT COUNT(*) answered, COALESCE(SUM(r.correct),0) correct FROM results r JOIN questions q ON q.id=r.question_id WHERE r.user_id=? AND q.bank_id=?',
        req.user.id,
        b.id,
      ),
      resume:
        one(
          'SELECT id, cursor, mode, question_ids FROM practice WHERE user_id=? AND bank_id=? AND completed=0 ORDER BY updated DESC LIMIT 1',
          req.user.id,
          b.id,
        ) || null,
    }));
    const summary = one(
      'SELECT COUNT(*) answered,COALESCE(SUM(correct),0) correct FROM results WHERE user_id=?',
      req.user.id,
    );
    const attempts = one(
      'SELECT COUNT(*) total,COALESCE(SUM(correct),0) correct FROM attempts WHERE user_id=?',
      req.user.id,
    );
    res.json({
      admin: req.user.admin_until > Date.now(),
      banks,
      summary,
      attempts,
    });
  ...
  app.post('/api/progress', (req, res) => {
    ...
    const selected = [
      ...new Set(
        Array.isArray(req.body?.selected) ? req.body.selected.map(String) : [],
      ),
    ].sort();
    const correct =
      JSON.stringify(selected) === JSON.stringify(JSON.parse(q.answer).sort());
    run(
    ...
    run(
      'INSERT INTO results VALUES(?,?,?,?) ON CONFLICT(user_id,question_id) DO UPDATE SET correct=excluded.correct,updated=excluded.updated',
      req.user.id,
      q.id,
      Number(correct),
      Date.now(),
  ...
  function practiceData(req, id) {
    const p = one(
      'SELECT * FROM practice WHERE id=? AND user_id=?',
      id,
      req.user.id,
    );
    if (!p) throw fail('练习不存在', 404);
    const ids = JSON.parse(p.question_ids),
      answers = JSON.parse(p.answers);
    return {
      ...p,
      answers,
      drafts: JSON.parse(p.drafts),
      question_ids: undefined,
      user_id: undefined,
      questions: ids.map((id) => {
        const q = decode(one('SELECT * FROM questions WHERE id=?', id));
        return { ...q, answer: undefined, explanation: undefined };
      }),
      bank_name: one('SELECT name FROM banks WHERE id=?', p.bank_id).name,
    };
  }
  app.post('/api/practice', (req, res) => {
    const { bankId, type, category, mode = '顺序练习' } = req.body;
    if (!one('SELECT id FROM banks WHERE id=?', String(bankId)))
      throw fail('题库不存在', 404);
    if (!['顺序练习', '随机练习', '错题练习'].includes(mode))
      throw fail('练习模式不正确');
    let rows = all(
      'SELECT id,type,category FROM questions WHERE bank_id=? ORDER BY position',
      bankId,
    ).filter(
      (q) =>
        (!type || q.type === type) && (!category || q.category === category),
    );
    if (mode === '错题练习') {
      const wrong = new Set(
        all(
          'SELECT question_id FROM results WHERE user_id=? AND correct=0',
          req.user.id,
        ).map((r) => r.question_id),
      );
      rows = rows.filter((q) => wrong.has(q.id));
    }
    if (mode === '随机练习')
    ...
    const id = randomUUID();
    run(
      'INSERT INTO practice(id,user_id,bank_id,mode,question_ids,updated) VALUES(?,?,?,?,?,?)',
      id,
      req.user.id,
      bankId,
      mode,
      JSON.stringify(rows.map((q) => q.id)),
      Date.now(),
    );
    res.json(practiceData(req, id));
  });
  app.get('/api/practice/:id', (req, res) =>
    res.json(practiceData(req, req.params.id)),
  );
  app.patch('/api/practice/:id', (req, res) => {
    const p = practiceData(req, req.params.id),
      cursor = req.body.cursor;
    if (!Number.isInteger(cursor) || cursor < 0 || cursor >= p.questions.length)
      throw fail('题目位置不正确');
    const drafts = { ...p.drafts };
    if (req.body.selected) drafts[p.questions[cursor].id] = req.body.selected;
    run(
      'UPDATE practice SET cursor=?,drafts=?,updated=? WHERE id=?',
      cursor,
  ...
  app.post('/api/practice/:id/answer', (req, res) => {
    ...
    if (!p.questions.some((q) => q.id === qid))
      throw fail('题目不在当前练习中');
    if (p.answers[qid]) return res.json(p.answers[qid]);
    const q = decode(one('SELECT * FROM questions WHERE id=?', qid));
    const selected = [
      ...new Set(Array.isArray(req.body.selected) ? req.body.selected : []),
    ].sort();
    if (
      !selected.length ||
      selected.some(
        (s) =>
          typeof s !== 'string' ||
          !/^[A-H]$/.test(s) ||
          s.charCodeAt(0) - 65 >= q.options.length,
      ) ||
      (q.type !== '多选题' && selected.length !== 1)
    )
      throw fail('请选择有效答案');
    const correct =
      JSON.stringify(selected) === JSON.stringify(q.answer.sort());
    const result = {
      selected,
      correct,
      answer: q.answer,
    ...
    transact(() => {
      ...
      run(
        'UPDATE practice SET answers=?,updated=? WHERE id=?',
        JSON.stringify({ ...p.answers, [qid]: result }),
        Date.now(),
        p.id,
      );
    });
  ...
  app.post('/api/practice/:id/finish', (req, res) => {
    const p = practiceData(req, req.params.id);
    if (Object.keys(p.answers).length !== p.questions.length)
      throw fail('请完成全部题目后结束练习');
    run(
      'UPDATE practice SET completed=1,updated=? WHERE id=?',
      Date.now(),
      p.id,
    );
    res.json({ ok: true });
  ...
  app.get('/api/template', async (req, res) => {
    ...
    res.set('Content-Disposition', 'attachment; filename=quiz-template.xlsx');
    res.send(Buffer.from(await template()));
  });
  app.post(
    '/api/admin/preview',
    admin,
    ...
    async (req, res) => {
      if (!req.file || !/\.xlsx$/i.test(req.file.originalname))
        throw fail('请选择 .xlsx 文件');
      try {
        res.json(await parseWorkbook(req.file.buffer));
      } catch (error) {
        throw fail(`无法导入：${error.message}`);
      }
    },
  );
  app.post('/api/admin/banks', admin, (req, res) => {
    const name = String(req.body.name || '').trim();
    if (!name || name.length > 100) throw fail('题库名称需为 1—100 字');
    if (
      !Array.isArray(req.body.questions) ||
      !req.body.questions.length ||
      req.body.questions.length > 5000
    )
      throw fail('每次导入 1—5000 道题');
    const qs = req.body.questions.map(validateQuestion);
    res.json({ id: addBank(name, qs), count: qs.length });
  });
  app.get('/api/admin/banks/:id', admin, (req, res) =>
    res.json(
      all(
        'SELECT * FROM questions WHERE bank_id=? ORDER BY position',
        req.params.id,
      ).map(decode),
    ),
  );
  app.put('/api/admin/questions/:id', admin, (req, res) => {
    const q = validateQuestion(req.body),
      old = one('SELECT bank_id FROM questions WHERE id=?', req.params.id);
    if (!old) throw fail('题目不存在', 404);
    transact(() => {
      run(
        'UPDATE questions SET type=?,prompt=?,options=?,answer=?,explanation=?,category=?,difficulty=? WHERE id=?',
        q.type,
        q.prompt,
        JSON.stringify(q.options),
        JSON.stringify(q.answer),
      ...
      run('DELETE FROM practice WHERE bank_id=?', old.bank_id);
      run('DELETE FROM results WHERE question_id=?', req.params.id);
    });
    res.json({ ok: true });
  });
  app.delete('/api/admin/banks/:id', admin, (req, res) => {
    run('DELETE FROM banks WHERE id=?', req.params.id);
    res.json({ ok: true });
  });
